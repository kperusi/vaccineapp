...
export default function PointDashboard() {
  ...
  const { rejectedRequest, request, allocations, facility, loading, stock, pendingRequest,approvedRequest } =
  ...
  useEffect(() => {
    ...
    const localUser = JSON.parse(localStorage.getItem("vax-login-user"));
              ...
              {pendingRequest?.map((item) => {
                ...
                return (
                  <tr key={item.id}>
                    ...
                    <td className="text-l">{item.note}</td>
